...
struct s_kmer_t{
   ...
   char l_ext;
   char r_ext;
   int64_t next; // Store the index of next
};
...
void s_add_kmer( shared int64_t* hashtable, int tablesize, shared s_kmer_t * memory_heap, int64_t heap_pos,
             const unsigned char *kmer, char left_ext, char right_ext)
{
   ...
   tmp.l_ext = left_ext;
   tmp.r_ext = right_ext;
   upc_memput(&memory_heap[heap_pos], &tmp, sizeof(s_kmer_t));
...
int main(int argc, char *argv[]){
	...
	int64_t posInContig, contigID = 0, totBases = 0, ptr = 0;
    ...
    fprintf(stderr,"nKmers = %lld\n",nKmers);

    /* Read the kmers from the input file and store them in the working_buffer */
    const int nKmers_per_thread = (nKmers + THREADS - 1) / THREADS;
    int nKmers_local = nKmers_per_thread;
    if(MYTHREAD*nKmers_per_thread >= nKmers) nKmers_local = 0;
    else if((MYTHREAD+1)*nKmers_per_thread >= nKmers) nKmers_local = nKmers - MYTHREAD*nKmers_per_thread;

    int nKmers_before = MYTHREAD * nKmers_per_thread;
    ...
    int64_t n_buckets = nKmers * LOAD_FACTOR;
   	...
   	//fprintf(stderr,"nKmers = %lld, n_buckets = %lld\n",nKmers,n_buckets);
   	//printf("nKmers = %lld, n_buckets = %lld\n",nKmers,n_buckets);
	...
	int64_t i;
  	upc_forall(i=0; i<n_buckets; i++; &hashtable[i]) 
   	...
   	//printf("nKmers_per_thread = %lld, THREADS = %lld\n",nKmers_per_thread,THREADS);
   	int64_t heap_pos = 0;
	...
	for(int64_t curStartNode = 0; curStartNode < start_list_size; curStartNode++){
	  ...
	  while (right_ext != 'F') {
	     ...
	     cur_kmer_idx = s_lookup_kmer(hashtable, n_buckets, memory_heap, (const unsigned char *) &cur_contig[posInContig-KMER_LENGTH]);
